...
1. **Does the final `snow_solver_fun!(constraint_values, xopt)` call run a full propagation?**
   - YES - `solver_fun!` calls `apply_event` for all events, which includes propagation
   - This will populate `spacecraft.history.segments` with the solution (flags restored before this call)
   - Verified: This is the correct pattern

2. **History clearing:**
   - Spec says "do NOT clear existing history"
   - If user runs solve multiple times, history accumulates
   - **Decision:** Document this behavior - user can manually clear if needed
   - Consider adding `clear_history!()` helper in future

3. **Flag restoration timing:**
