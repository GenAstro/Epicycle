...
using SciMLBase: CallbackSet, ODESolution
using LinearAlgebra
...
using AstroProp
using AstroProp: compute_point_mass_gravity!   # the kernel under PointMassGravity, tested directly
...
@testset "Point Mass Gravity Propagation -Earth" begin
    ...
    propagate!(prop, sat, StopAt(sat, PropDurationDays(), 10.0))
...
@testset "Point Mass Gravity Propagation -Moon" begin
    ...
    propagate!(prop, sat, StopAt(sat, PropDurationDays(), 5.0))
...
include("runtests_testgaps.jl")
include("force_drag_exponential.jl")
include("force_gravity_zonal.jl")
