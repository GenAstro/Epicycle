...
"""
    STMConfig(; Φ = false, S_p = ModelVariable[])
...
"""
    OrbitODEProblem(prop, sc; duration_s, stm = nothing, dense = false)
...
- `sol::Any` — raw `ODESolution` if `prob.dense == true`, otherwise
  ...
  the concrete ODE solution type.
"""
...
# ---------------------------------------------------------------------------
# solve  (extends CommonSolve.solve via the import in AstroProp.jl)
# ---------------------------------------------------------------------------
...
function _augmented_solve(prob::OrbitODEProblem, y0, tspan, start_epoch)
    ...
    # Allocate the Jacobian result once and reuse it at every RHS call
